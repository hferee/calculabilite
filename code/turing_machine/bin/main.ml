...
let print_tape tape =
  tape.left_tape |> List.rev |> print_list;
  tape.right_tape |> print_list;
  print_newline ();
  tape.pos |> string_of_int |> print_endline
...
let step turing_machine =
  print_tape turing_machine.tape;
  let state, letter =
...
let run turing_machine words =
  ...
  let tm =
    {
      turing_machine with
      tape = init_tape words;
      current_state = turing_machine.initial;
    }
  in
...
let example_machine =
  let transition s w =
    ...
    | Q3, None -> failwith "Nope"
    (* *)
    | _, Some w -> failwith ("Some " ^ w)
    | _, None -> failwith "none"
  in
  {
    initial = Q0;
    rejet = Q3;
...
let () =
  match run example_machine [ "0"; "0"; "1"; "1" ] with
  | Yes -> print_char 'y'
  | No -> print_char 'n'
